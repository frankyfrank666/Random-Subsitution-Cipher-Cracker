function [accept_new_key, prob_accept] = metropolis(current_key, new_key, pr_trans, encrypted_txt)
    decryptedmsg = Decrypt(encrypted_txt,current_key);
    logn_pr_curr = logn_pr_txt(decryptedmsg,pr_trans);
    
    decryptedmsg = Decrypt(encrypted_txt,new_key);
    logn_pr_new = logn_pr_txt(decryptedmsg,pr_trans);
